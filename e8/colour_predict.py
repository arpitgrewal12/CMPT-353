...
def main(infile):
    ...
    #Extracting colours as y values
    ...
    #Converting rgb colours to lab colours
    def func(X):
        X = X.reshape(1,-1,3)
        X = rgb2lab(X)
        X = X.reshape(-1,3)
    ...
    # Idea taken from From lecture notes
    bayes_convert_model = make_pipeline(
        FunctionTransformer(func, validate= True),
        GaussianNB()
    )
    
    knn_rgb_model= KNeighborsClassifier(n_neighbors=11)
    ...
    rf_rgb_model=RandomForestClassifier(n_estimators=100,max_depth=None,min_samples_leaf=10)
    
    rf_convert_model=make_pipeline(
           FunctionTransformer(func, validate= True),
           RandomForestClassifier(n_estimators=100,max_depth=None,min_samples_leaf=10)
    )
